...
async function loadDashboard() {
    try {
        ...
        console.log("📍 Versuche Contracts zu laden von: contracts/v2/" + DEV_UID);
        const contractsRef = db.collection('contracts').doc('v2').collection(DEV_UID);
...
async function loadCharts() {
    try {
        ...
        const typesRef = db.collection('contracts').doc('v2').collection(DEV_UID);
...
async function editContract(contractId) {
    try {
        const contractDoc = await db.collection('contracts').doc('v2').collection(DEV_UID).doc(contractId).get();
...
async function editCustomer(contractId, currentCustomer) {
    ...
    if (newCustomer !== null && newCustomer !== '') {
        try {
            await db.collection('contracts').doc('v2').collection(DEV_UID).doc(contractId).update({
...
async function editCustomer2025(contractId, currentCustomer) {
    ...
    if (newCustomer !== null && newCustomer !== '') {
        try {
            await db.collection('contracts').doc('v2').collection(LIVE_UID).doc(contractId).update({
                customer: newCustomer
            });
...
// ==================== CONTRACTS 2025 ====================
const LIVE_UID = 'QXyzCIkPR6TU2ZRPYAz1UKpotXz2';

async function refreshContracts2025() {
    try {
        // Lade Email des Benutzers und update Tab-Titel
        const userDoc = await db.collection('users').doc(LIVE_UID).get();
        if (userDoc.exists) {
            const userEmail = userDoc.data().email || LIVE_UID;
...
async function editContract2025(contractId) {
    try {
        const contractDoc = await db.collection('contracts').doc('v2').collection(LIVE_UID).doc(contractId).get();
        if (contractDoc.exists) {
            const data = contractDoc.data();
            const newPrice = prompt('Neuer Preis:', data.price);
            if (newPrice !== null && !isNaN(newPrice)) {
    ...
    try {
        ...
        if (snapshot.empty) {
            ...
            snapshot.forEach(doc => {
                ...
                const userId = data.user || DEV_UID;
...
async function loadPlanPositions() {
    try {
        ...
        if (!tbody) {
            console.log('ℹ️ loadPlanPositions skipped - #planPositionsTable nicht in HTML');
            return;
        }
